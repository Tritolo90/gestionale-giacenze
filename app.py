# app.py (versione finale All-in-One per il deploy)
...
FILE_LAST_UPDATE = 'last_update.txt' # File per memorizzare l'ultimo aggiornamento
...
def process_all_data(status_placeholder):
    ...
    Fornisce aggiornamenti di stato all'interfaccia.
    """
    
    # --- Funzioni di supporto interne ---
    def rinomina_file_sap_in_txt(folder_path):
        status_placeholder.write(f"... Controllo/rinomina file in: {folder_path}")
            ...
            if fields[0].strip().isdigit() and len(fields) > 6:
                try:
                    clean_data_rows.append({
                        'Materiale': fields[0].strip(), 'mag': current_mag,
                        'Descrizione': fields[1].strip(), 'Qtà Disponibile': fields[6].strip()
                    })
    ...
    status_placeholder.update(label="Fase 1/2: Elaborazione dati di dettaglio (Digigem e NAV)...")
    ...
    status_placeholder.update(label="Fase 2/2: Elaborazione dati aggregati (SAP, Digigem, NAV)...")
    ...
    final_cols_riepilogo = ['Materiale', 'Provincia', 'Descrizione', 'Qtà Disponibile(SAP)', 'Qtà Digigem', 'Delta(Digigem - SAP)', 'NAV.Giacenza', 'VIAGGIANTE (NAV - SAP)']
    df_riepilogo = df_summary[[c for c in final_cols_riepilogo if c in df_summary.columns]]
...
# Inizializza lo stato della sessione per tenere traccia dei dati
...
with col1:
    if st.button("🔄 Carica e Processa Dati"):
        with st.status("Elaborazione in corso...", expanded=True) as status:
            df_d, df_r = process_all_data(status)
...
if st.session_state.data_loaded:
    ...
    st.info("Clicca sul pulsante in alto per caricare e processare i dati per la prima volta.")